...
class Evaluator:
    ...
    def load_inferencer_output(self) -> None:
        ...
        if not os.path.exists(self.INPUT_FILE_NAME):
            raise FileNotFoundError(f"Input file {self.INPUT_FILE_NAME} does not exist.")
        ...
        if isinstance(dataset, (DatasetDict, IterableDatasetDict, IterableDataset)):
            raise NotImplementedError("Loading from IterableDataset or DatasetDict is not supported yet.")
        for required_column in required_columns:
            if required_column not in dataset.column_names:
                raise ValueError(f"Missing required column: {required_column} in the input dataset.")
    ...
    def load_embeddings(self):
        ...
        if self.embeddings is None:
            raise ValueError(f"Failed to load embeddings model: {self.config.embedding}")
    ...
    def set_ragas_metrics(self) -> None:
        ...
        if not all(metric in self.SUPPORTED_RAGAS_METRICS for metric in self.config.metrics):
            unsupported_metrics = [metric for metric in self.config.metrics if metric not in self.SUPPORTED_RAGAS_METRICS]
            raise ValueError(f"Unsupported metrics found in configuration: {unsupported_metrics}")
        # Return the list of Ragas metrics functions (that are `called`) based on the configuration
        self.metrics = [self.SUPPORTED_RAGAS_METRICS[metric]() for metric in self.config.metrics]
    ...
    def get_summary_results(self) -> dict:
        ...
        if self.evaluation_results is None:
            raise ValueError("Evaluation results are not available. Please run the evaluation first.")
    ...
    def save_results(self) -> None:
        ...
        if not self.OUTPUT_FILE_NAME_SUMMARY.endswith(".json"):
            raise NotImplementedError("Only JSON format is supported for summary results.")
        summary = self.evaluation_results._repr_dict
    ...
    def run(self):
        ...
        self.load_llm()
        print(f"--- ✅ Loaded embeddings model: {self.config.embedding} and LLM: {self.config.llm} ---")
        ...
        # Save results
        self.save_results()
        print(f"--- ✅ Results saved to {self.OUTPUT_FILE_NAME_DETAILED} and {self.OUTPUT_FILE_NAME_SUMMARY} ---")
